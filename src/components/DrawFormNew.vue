      ...
      <div class="form-block">
        ...
        <div class="sources">
          <label
            ...
            :key="s"
            class="chip"
            :class="{ on: form.sources.includes(s) }"
          >
...
const selectAll = () => {
  form.sources = [...allSources] as unknown as any
}
const clearAll = () => {
  form.sources = []
}
...
.form-group {
  ...
  gap: 6px;
  height: 100%; /* Ensure all form groups take equal height */
}
.form-group label {
  font-weight: 600;
  min-height: 20px; /* Fixed height for labels */
  display: flex;
...
.form-group small {
  color: #6b7280;
  font-size: 0.8rem;
  min-height: 16px; /* Fixed height for hint text */
  display: flex;
  align-items: flex-start; /* Align hint text to top */
}
