...
type Card = { id: number; event: string; title: string; text: string; meta?: string; kind: 'info'|'ok'|'err' }
...
const start = () => {
  ...
  try {
    ...
    ws.onmessage = (ev) => {
      try { const msg = JSON.parse(ev.data); if (msg?.event) events.value.push({ event: msg.event, data: msg.data }) }
      catch { /* ignore */ }
    }
    ws.onerror = () => { fallbackSSE() }
  } catch {
    fallbackSSE()
  }
}
...
const toReadable = (event: string, data: any): { title: string; text: string; kind: Card['kind']; meta?: string } => {
  const kind: Card['kind'] = /error|fail/.test(event) ? 'err' : (/final|done|completed/.test(event) ? 'ok' : 'info')
  if (event === 'ready') {
    const scenario = typeof data?.scenario === 'string' ? data.scenario : ''
    return { title: 'Готово к старту', text: scenario ? `Сценарий: ${scenario}` : 'Демонстрационный режим', kind }
  }
  if (event.startsWith('demo:')) {
    const stage = event.split(':')[1] || data?.stage?.split(':')[1] || 'stage'
    const map: Record<string,string> = {
      start: 'Старт демо', entropy: 'Сбор энтропии', whitening: 'Вайтинг', seed: 'Формирование seed', draw: 'Генерация', tests: 'Тесты NIST', finish: 'Завершение',
    }
  ...
  if (event === 'final') {
    ...
    return { title: 'Финал', text: ok ? 'Завершено успешно' : `Статус: ${data?.status ?? '—'}`, kind: ok ? 'ok' : 'err' }
  }
  // generic fallback
...
.bricks { display: flex; flex-direction: column; gap: 12px; }
.brick { background: #fff; border: 1px solid #e5e7eb; border-radius: 10px; padding: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.04); }
.brick.ok { border-color: #bcd0c7; }
.brick.err { border-color: #f3b9bd; }
.brick .title { font-weight: 700; color: #111827; margin-bottom: 4px; }
.brick .text { color: #374151; }
.brick .meta { color: #6b7280; font-size: 12px; margin-top: 6px; }

/* Animations */
.brick-enter-active, .brick-leave-active { transition: all .18s ease; }
.brick-enter-from { opacity: 0; transform: translateY(6px); }
.brick-leave-to { opacity: 0; transform: translateY(6px); }
</style>
