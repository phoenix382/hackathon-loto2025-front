...
<template>
  <div class="draw">
    <section class="hero">
      ...
      <p>Псевдослучайные числа из комбинированной энтропии: источники → нормализация → seed → генерация → тесты.</p>
      <div class="badges">
      ...
      <div class="steps">
        <div class="step" :class="{ active: stepIndex === 1 }"><span class="dot">1</span> Настройка</div>
        <div class="step" :class="{ active: stepIndex === 2 }"><span class="dot">2</span> В процессе</div>
        <div class="step" :class="{ active: stepIndex === 3 }"><span class="dot">3</span> Результат</div>
      </div>
    </section>
    ...
    <section class="grid">
      ...
      <div class="col">
        ...
        <div v-if="state.result" class="card sticky-card">
          ...
          <div class="info">
            <div class="row"><span class="k">ID задания:</span><span class="v mono">{{ state.result.job_id }}</span></div>
            <div class="row"><span class="k">Начато:</span><span class="v">{{ ts(state.result.started_at) }}</span></div>
            <div class="row" v-if="state.result.finished_at"><span class="k">Завершено:</span><span class="v">{{ ts(state.result.finished_at as number) }}</span></div>
            <div class="row"><span class="k">Отпечаток:</span><span class="v mono">{{ state.result.fingerprint }}</span></div>
          </div>
          ...
          <div v-if="state.showBits && state.bits" class="bits" ref="bitsEl">
            ...
            <div class="actions">
              <button class="btn ghost" @click="saveBitsAsTxt" :disabled="!state.bits">Скачать bits.txt</button>
            </div>
          </div>
        </div>
      </div>
...
.steps { display: flex; gap: 10px; justify-content: center; margin-top: 10px; flex-wrap: wrap; }
.step { display: inline-flex; align-items: center; gap: 8px; padding: 6px 12px; border-radius: 999px; border: 1px solid #e5e7eb; color: #374151; background: #fff; font-size: 13px; }
.step .dot { width: 20px; height: 20px; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center; background: #e5e7eb; color: #111827; font-weight: 700; font-size: 12px; }
.step.active { border-color: #0d6efd; color: #0d6efd; }
.step.active .dot { background: #0d6efd; color: #fff; }
...
.ball { width: 56px; height: 56px; border-radius: 50%; background: #0d6efd; color: #fff; display: inline-flex; align-items: center; justify-content: center; font-weight: 800; box-shadow: 0 2px 8px rgba(0,0,0,0.12); animation: pop-in .35s cubic-bezier(.2,.7,.2,1) both; }
